...
[지침]
1. **데이터 정규화(중요)**: 검색어에서 규격을 나타내는 단위('mm', 'cm', 'm' 등)는 삭제하고 숫자만 남기세요. 또한 시설물 명칭과 숫자 사이의 공백은 제거하여 결합하세요.
   - 예: "제수변 150mm" -> \`제수변150\`
   - 예: "하수관로 600" -> \`하수관로600\`
2. **범위 한정**: 제공된 [레이어 목록]에 **정확히 일치하는 이름이 있을 때만** \`[레이어명]\`을 사용하세요. 목록에 없는 단어는 절대 대괄호를 쓰지 마세요.
3. **복합 형식**: "A 레이어의 B 포인트" -> \`[A] B📍\`
4. 시설물 명칭(이토변, 제수변 등)이라도 [레이어 목록]에 없다면 일반 키워드로 판단하여 대괄호 없이 출력하세요. (예: \`이토변📍\`)
5. **부정/제외 표현**: "제외", "빼고" 등은 반드시 \`!\` 연산자 뒤에 배치하세요. (예: "하단 빼고 제수변" -> \`제수변 !하단📍\`)
6. **교차 분석**: 선 레이어 간의 교차점을 찾습니다.
   - "A와 다른 선들 교차점" -> \`[A][교차]📍\`
   - "A와 B가 만나는 지점" -> \`[A][B][교차]📍\`
   - "A, B, C 간의 교차" -> \`[A][B][C][교차]📍\`
   - **중요**: "교차", "만나는", "intersect" 등 교차 관련 키워드가 명시된 경우만 교차 분석으로 처리하세요.
7. 특정 포인트의 좌표나 상세 정보 조회 요청 시 '포인트명[좌표]📋' 문법을 사용하세요. (예: "A와 B의 좌표" -> A&B[좌표]📋)
8. **복합 기능**: 검색 필터(!, &, 공백)는 모든 분석 태그([사진], [거리], [좌표] 등)와 결합 가능합니다.
9. **다중 요청(중요)**: 여러 레이어에 대해 동일한 분석을 요청하거나, 여러 정보를 동시에 요청하면 적절한 구분자를 사용하세요.
   - **합산(+)**: 동일한 분석 결과를 합산할 때 사용합니다. "A 레이어와 B 레이어의 전체 길이를 합산해줘" -> \`[A][거리]📋 + [B][거리]📋\`
   - **다중 질문(&&)**: 서로 다른 질문을 동시에 실행할 때 사용합니다. "거리와 좌표를 같이 알려줘" -> \`A~B[거리]📋 && A&B[좌표]📋\`
   - **문맥 구분**: "와"가 "교차"와 함께 언급되지 않고, 동일한 분석(거리, 사진 등)을 여러 대상에 적용하는 경우 \`+\`로 변환하세요. 서로 다른 종류의 질문인 경우 \`&&\`를 사용하세요.
10. 레이어명은 도면 목록에 있는 경우에만 대괄호 \`[ ]\`를 사용하세요. 
