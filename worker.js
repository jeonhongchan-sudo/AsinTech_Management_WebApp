...
**[핵심 미션 - 절대 규칙]**
1. **오직 변환된 문법 결과만 반환하세요. 어떤 다른 설명, 인사말, 부연설명도 절대 추가하지 마세요.**
2. **검색어 정규화**: 수치 뒤의 단위('mm', 'cm' 등)는 제거하고 숫자만 남기세요. 명칭과 숫자 사이의 공백도 제거하여 결합하세요. (예: "제수변 150mm" -> 제수변150)
3. **레이어명 일치 및 대괄호 사용**: 사용자가 언급한 대상이 아래 **[대상 레이어 목록]** 에 **정확히 일치하는 이름일 경우에만 대괄호 [레이어명]을 사용하세요.** 목록에 없는 이름은 절대 대괄호를 씌우지 마세요.
4. **복합 검색 예시**: "X 레이어에서 Y를 찾아줘"와 같은 요청은 **[X] Y📍** 형식으로 변환하세요.
**[기본 문법 규칙]**
1. [레이어명]📋 : 리스트 분석 출력
2. [레이어명]📍 : 지도 위치 표시
3. [레이어명][거리]📋 : 총 연장 계산
...
7. [레이어1][레이어2...][교차]📋 : 다중 레이어 간 교차점 분석
8. 키워드1 키워드2📍 : **AND 검색**
9. 키워드1 & 키워드2📍 : **OR 검색**
10. 키워드1 !키워드2📍 : **NOT 검색**

[★ 다중 작업 연산자 기호 판단 규칙 ★]
문장에 여러 레이어나 복수 요청이 결합되어 있을 때, 사용자의 논리적 의도를 **가장 정확하게 파악하여** 기호를 선택하세요.
1. '+' 연산자 (산술적 합산): (예: "A와 B의 연장 합계") -> **[A][거리] + [B][거리]📋**
   - 올바른 변환 예시: "레이어1과 레이어2의 전체 길이 합산" -> **[레이어1][거리] + [레이어2][거리]📋**
2. '&&' 연산자 (독립적인 다중 결과 병렬 출력): (예: "A 리스트 뽑고 B는 지도에 찍어") -> **[A]📋 && [B]📍**
   - 올바른 변환 예시: "레이어1 리스트와 레이어2 사진 매칭 분석" -> **[레이어1]📋 && [레이어2][사진]📋**

**[대상 레이어 목록]** (현재 도면에 존재하는 레이어들 - **반드시 이 목록을 활용하여 레이어명을 변환하세요.**)
${searchContext}`;
...
[절대 규칙]
1. **[도면 데이터 검색 결과 요약] 텍스트에 있는 실제 수치만 사용하세요 (할루시네이션(환각) 절대 금지).**
2. **인사말 없이, 아래 JSON 형식으로만 끝까지 작성하여 출력하세요. 불완전한 JSON도 안 됩니다. 완전하고 정확한 JSON만 반환하세요.**

**[출력 JSON 형식 - 반드시 이 형식을 지키세요]**
{
  "intent": "질문 의도 요약",
