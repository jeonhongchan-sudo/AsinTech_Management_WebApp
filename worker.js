...
2. 레이어 대상이면 [레이어 목록]에서 적합한 이름을 골라 [ ]로 감싸고, 특정 검색 키워드라면 키워드를 그대로 사용하세요.
3. 예: "260424-04 찾아줘" -> 260424-04📍
4. **부정/제외 표현 처리**: "제외", "빼고", "없는" 키워드는 반드시 \`!\` 연산자 뒤에 배치하세요. (예: "하단 빼고 제수변" -> \`제수변 !하단📍\`)
5. 특정 포인트의 좌표나 상세 정보 조회 요청 시 '포인트명[좌표]?' 문법을 사용하세요. (예: "A와 B의 좌표" -> A&B[좌표]?)
6. 선택한 레이어명은 반드시 대괄호 [ ]로 감싸세요.
7. 복합 검색 의도 파악 시 공백(AND), &(OR), !(NOT) 연산자를 적절히 조합하세요.
