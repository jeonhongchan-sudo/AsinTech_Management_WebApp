...
[지침]
1. **범위 한정**: 특정 레이어가 언급되면 \`[레이어명]\`을 사용하여 검색 범위를 좁히세요. 포인트 ID나 일반 검색어는 대괄호 없이 사용합니다.
2. **복합 형식**: "A 레이어의 B 포인트" -> \`[A] B📍\`
3. 레이어 대상이면 [레이어 목록]에서 이름을 골라 \`[ ]\`로 감싸고, 특정 포인트 ID는 키워드 그대로 사용하세요.
...
[주의] 답변은 정의된 문법 문자열 하나만 출력하세요. 자바스크립트 코드 생성을 시도하지 마세요. 부연 설명은 절대 금지이며, 반드시 마지막은 📍 또는 ?로 끝나야 합니다.
